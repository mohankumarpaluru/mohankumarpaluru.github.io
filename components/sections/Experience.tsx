                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 bg-slate-50 dark:bg-black/70 backdrop-blur-md p-4 md:p-8 rounded-2xl border border-slate-200 dark:border-white/5 shadow-2xl h-full overflow-hidden transition-colors duration-500">
                    ...
                    <div className="flex flex-col min-h-0">
                        ...
                        <div className="flex space-x-1 bg-slate-100 dark:bg-black/40 p-1 rounded-lg border border-slate-200 dark:border-white/10 w-fit mb-4 shrink-0 self-start md:self-auto transition-colors">
                            <button onClick={() => setActiveTab('experience')} className={`px-4 py-1.5 text-xs font-mono rounded-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500/50 ${activeTab === 'experience' ? 'bg-white/80  dark:bg-white/10 text-cyan-600 dark:text-cyan-400 shadow-sm font-bold' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 hover:bg-white/50 dark:hover:bg-white/5'}`}>EXPERIENCE</button>
                            <button onClick={() => setActiveTab('education')} className={`px-4 py-1.5 text-xs font-mono rounded-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500/50 ${activeTab === 'education' ? 'bg-white/80  dark:bg-white/10 text-cyan-600 dark:text-cyan-400 shadow-sm font-bold' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-300 hover:bg-white/50 dark:hover:bg-white/5'}`}>EDUCATION</button>
                        </div>
                            ...
                            {activeTab === 'experience' && (
                                <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
                                    {portfolioData.experience.map((exp, i) => (
                                        <GlowCard
                                            key={i}
                                            className="p-4 md:p-5 overflow-hidden relative group hover:-translate-y-1 hover:shadow-lg transition-all duration-300 focus-within:ring-2 focus-within:ring-blue-500/50 rounded-xl"
                                            tabIndex={0}
                                            role="article"
                                        >
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <div className="text-blue-600 dark:text-blue-400 text-xs md:text-sm font-medium mb-3">{exp.company}</div>
                            ...
                            {activeTab === 'education' && (
                                <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
                                    {portfolioData.education.map((edu, i) => (
                                        <GlowCard
                                            key={i}
                                            className="p-4 overflow-hidden relative group hover:shadow-md transition-all focus-within:ring-2 focus-within:ring-blue-500/50"
                                            tabIndex={0}
                                            role="article"
                                        >
                                            <div className={cardScrim}></div>
                                            <div className="relative z-10 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                                                <div>
                                                    ...
                                                    <div className="text-sm text-blue-600 dark:text-blue-400 mt-1">{edu.institution}</div>
                                                </div>
