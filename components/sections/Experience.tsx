...
export const Experience: React.FC = () => {
  ...
  return (
    <Section index={PortfolioState.EXPERIENCE}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 bg-white/50 dark:bg-black/70 backdrop-blur-md p-4 md:p-8 rounded-2xl border border-slate-200 dark:border-white/5 shadow-2xl h-full overflow-hidden transition-colors duration-500">
            ...
            <div className="flex flex-col shrink-0 md:shrink min-h-0 overflow-y-auto md:overflow-visible no-scrollbar">
                ...
                {/* Permanent Certifications Block */}
                        ...
                        {portfolioData.certifications.map((cert, i) => (
                            <div key={i} className="flex flex-col border-l-2 border-slate-300 dark:border-white/10 pl-4 hover:border-cyan-500/50 transition-colors py-1 group cursor-default">
                                ...
                                <div className="flex justify-between items-center mt-1">
                                    ...
                                    <span className="text-[10px] text-slate-500 font-mono border border-slate-200 dark:border-white/5 bg-white/50 dark:bg-white/5 px-1.5 py-0.5 rounded group-hover:border-cyan-500/20 transition-colors">{cert.year}</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="flex flex-col min-h-0">
                ...
                <div className="flex space-x-1 bg-slate-100 dark:bg-black/40 p-1 rounded-lg border border-slate-200 dark:border-white/10 w-fit mb-4 shrink-0 self-start md:self-auto transition-colors">
                    <button
                        ...
                        className={`px-4 py-1.5 text-xs font-mono rounded-md transition-all duration-300 ${
                            activeTab === 'experience' 
                            ? 'bg-white dark:bg-white/10 text-cyan-600 dark:text-cyan-400 shadow-sm font-bold' 
                    ...
                    {activeTab === 'experience' && (
                        <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
                            {portfolioData.experience.map((exp, i) => (
                                <GlowCard key={i} className="p-4 md:p-5 overflow-hidden relative">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <span className="text-xs font-mono text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-white/5 px-2 py-1 rounded mt-1 md:mt-0 whitespace-nowrap border border-slate-200 dark:border-white/5">{exp.period}</span>
                                    </div>
                    ...
                    {activeTab === 'education' && (
                        <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-300">
                            {portfolioData.education.map((edu, i) => (
                                <GlowCard key={i} className="p-4 overflow-hidden relative group">
                                    <div className={cardScrim}></div>
                                    <div className="relative z-10 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                                        <div>
                                            <h4 className="text-base font-bold text-slate-800 dark:text-slate-200 group-hover:text-black dark:group-hover:text-white transition-colors">{edu.degree}</h4>
                                            <div className="text-sm text-indigo-600 dark:text-indigo-400 mt-1">{edu.institution}</div>
                                        </div>
                                        <div className="text-xs font-mono text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-white/5 px-2 py-1 rounded border border-slate-200 dark:border-white/5 shrink-0 self-start sm:self-center">
                                            {edu.year}
                                        </div>
                                    </div>
                                </GlowCard>
