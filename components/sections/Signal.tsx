            ...
            <div className="text-center w-full h-full flex flex-col justify-center">
                ...
                <div className="flex flex-col items-center gap-6 md:gap-8 font-mono text-sm">
                    <a href={`mailto:${portfolioData.bio.contacts.email}`} className="relative group inline-block">
                        <div className="absolute inset-0 bg-gradient-to-r from-teal-400 via-cyan-500 to-orange-500 rounded blur opacity-25 group-hover:opacity-75 transition-opacity duration-500"></div>
                    ...
                    <div className="flex flex-wrap justify-center gap-4 md:gap-8">
                        {Object.entries(portfolioData.bio.contacts.social).map(([key, url]) => (
                            <a key={key} href={url} target="_blank" rel="noreferrer" className="text-slate-500 dark:text-slate-400 hover:text-black dark:hover:text-white capitalize transition-colors text-sm md:text-lg hover:underline decoration-teal-500/50 underline-offset-4">
                                {key}
                            </a>
                        ))}
                    </div>
